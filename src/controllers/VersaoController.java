...
public class VersaoController {
    // atributos de classe
    private Repositorio repositorio = new Repositorio(BancoDados.consultaUsuario(Preferences.userRoot().get("emailUser", "")));
    ...
    public File getPastaVersoes() { return pastaVersoes; }

    // construtor
    public VersaoController(){ this.versao = new Versao();  }
    ...
    private void threadVersiona(String comentario) {
        ...
        new Thread() {
            ...
            public void run() {
                    ...
                    String hash = null;
                    try { hash = geraHashDaVersao(); } catch (NoSuchAlgorithmException e) {  e.printStackTrace(); }
                    // acessa a pasta ou cria, se não existir:
                    if(!getPastaVersoes().exists()) getPastaVersoes().mkdir();
                    // constroi objeto a ser inserido no banco de dados, no formato adequado. **** AINDA FALTA INSERIR O NOME,pois, não HÁ NO COOKIE
                    getVersao().setUsuario(emailCookie);  getVersao().setData(dataAtual());
                    getVersao().setVersao(hash); getVersao().setComentario(comentario);
    ...
    private ArrayList<String> conteudoRepositorio(){
        ...
        File arquivoContainerJson = new File(getRepositorio().getPath() + Arquivo.resolvePath() + "container" + Arquivo.resolvePath() + "container.json");
        // le container .json
    ...
    private void criaCopiaArquivosRepositorio(String hash){
        /* cria cópia dos arquivos do repositorio e move para a pasta de versões
         obs: O windows não permite copiar a pasta origem em si, por isso, é necessário criar
        ...
        if(conteudoRepositorio() != null){
            for(String caminhos: conteudoRepositorio()){
                if(new File(caminhos).isFile()){  // se for arquivo, gera um arquivo no destino e escreve nele
                    try {
                        if(Arquivo.resolvePath().equals("/")) Runtime.getRuntime().exec("cp " + caminhos + " " + destino);  // não testado no linux
                        else Runtime.getRuntime().exec("Xcopy " + caminhos + " " + destino );
                    } catch (IOException e) { e.printStackTrace(); }
                    ...
                    try { // cria a cópia
                        if(Arquivo.resolvePath().equals("/")) Runtime.getRuntime().exec("cp -R" + caminhos + " " + destino  + Arquivo.resolvePath() + new File(caminhos).getName()); // não testado no linux
                        else Runtime.getRuntime().exec("Xcopy /E /I " + caminhos + " " + destino + Arquivo.resolvePath() + new File(caminhos).getName());
                    } catch (IOException e) { e.printStackTrace(); }
                }
            }
        }
